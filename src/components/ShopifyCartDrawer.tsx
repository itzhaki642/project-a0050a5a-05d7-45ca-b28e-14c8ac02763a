...
export const ShopifyCartDrawer = () => {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent className="w-full sm:max-w-lg flex flex-col h-full" side="left">
        <SheetHeader className="flex-shrink-0">
          <SheetTitle>סל קניות</SheetTitle>
          <SheetDescription>
            {totalItems === 0 ? "הסל שלך ריק" : `${totalItems} פריט${totalItems !== 1 ? 'ים' : ''} בסל`}
          </SheetDescription>
        </SheetHeader>
        ...
        <div className="flex flex-col flex-1 pt-6 min-h-0">
          {items.length === 0 ? (
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto pl-2 min-h-0">
                <div className="space-y-4">
                  {items.map((item, index) => (
                    <div key={`${item.variantId}-${index}`} className="flex gap-4 p-2 border-b border-border">
